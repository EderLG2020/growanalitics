...
class UserController {
  static async getUsuarios(req, res) {
    try {
      ...
      if (view === "general") {
        ...
        if (idFilter) where.id = Number(idFilter);
        if (usuarioFilter) where.usuario = { contains: usuarioFilter };
        if (correoFilter) where.correo = { contains: correoFilter };
      } 
      else if (view === "roles") {
        where = {
          AND: [
      ...
      console.error("[X] Error en getUsuarios:", error);
      res.status(500).json({
  ...
  static async createUsuario(req, res) {
    try {
      ...
      res
        .status(201)
        .json({ message: "Usuario creado con éxito", data: userSinPassword });
    } catch (error) {
      if (error.code === "P2002") {
        ...
        return res
          .status(400)
          .json({ error: `El ${field} ya está registrado` });
      }
  ...
  static async login(req, res) {
    try {
      ...
      if (!user)
        return res
          .status(400)
          .json({ error: "Usuario o correo no encontrado" });
